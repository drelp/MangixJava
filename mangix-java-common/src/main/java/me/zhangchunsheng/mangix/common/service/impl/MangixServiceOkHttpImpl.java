 ...
 */
public class MangixServiceOkHttpImpl extends BaseServiceImpl {
    private static Logger logger = LoggerFactory.getLogger(MangixServiceOkHttpImpl.class);
    ...
    public String post(String url, Map<String, Object> params) throws MangixException {
        String requestStr = MangixServiceOkHttpImpl.getUrlParams(params);
        try {
            HttpClientBuilder httpClientBuilder = this.createHttpClientBuilder();
    ...
    public String postJson(String url, String requestStr, String privateToken) throws MangixException {
        try {
            ...
            Request request = new Request.Builder()
                    ...
                    .addHeader("Authorization", "Token " + privateToken)
                    .build();
            Response response = client.newCall(request).execute();
            if(response.code() == 200) {
                return response.body().string();
            } else {
                this.log.error("\n【请求地址】：{}\n【请求数据】：{}\n【异常信息】：{}", url, requestStr, response.message());
                if (this.getConfig().isIfSaveApiData()) {
                    mangixApiData.set(new MangixApiData(url, requestStr, null, response.message()));
                }
                throw new MangixException(response.message());
            }
        } catch (Exception e) {
            this.log.error("\n【请求地址】：{}\n【请求数据】：{}\n【异常信息】：{}", url, requestStr, e.getMessage());
            if (this.getConfig().isIfSaveApiData()) {
